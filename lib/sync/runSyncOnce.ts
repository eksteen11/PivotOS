...
import { db, type DbItem } from '@/lib/db/db'
...
export async function runSyncOnce(session: Session) {
  ...
  await syncEntitiesAndDivisions()
...
async function syncEntitiesAndDivisions() {
  const sb = supabase()!
  const { data: entities, error: eErr } = await sb
    .from('entities')
  ...
  const { data: workstreams, error: wErr } = await sb
    .from('workstreams')
...
async function pushDirtyItems(session: Session) {
  ...
  const { error } = await sb.from('items').upsert(toRemote, { onConflict: 'id' })
