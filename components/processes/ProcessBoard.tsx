          ...
          <ul className="mt-3 space-y-1 text-sm">
            {steps.slice(0, 4).map((s) => (
              <li key={s.id} className="flex items-center gap-2 text-muted">
                <span className={`h-1.5 w-1.5 rounded-full ${s.status === 'done' ? 'bg-accent' : 'bg-gray-300'}`} />
                {s.title}
              </li>
            ))}
          </ul>
      ...
      {columns.map((col) => {
        ...
        return (
          <div key={col.key} className="min-w-0">
            <h3 className="mb-3 text-sm font-semibold text-muted">{col.label} ({items.length})</h3>
